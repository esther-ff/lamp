...
impl IoSource {
    ...
    pub(crate) fn put_read_waker(&mut self, waker: &Waker) {
        self.wakers.put(waker, Direction::Read)
    }

    pub(crate) fn put_write_waker(&mut self, waker: &Waker) {
        self.wakers.put(waker, Direction::Write)
    }
}
